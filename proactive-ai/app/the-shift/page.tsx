...
export default function QuestionsPage() {
  ...
  // Handle question selection from TheShift - toggle behavior
  const handleQuestionSelect = (id: number | undefined) => {
    // If clicking the same question, close the panel
    if (id === selectedQuestionId) {
      setSelectedQuestionId(undefined);
    } else {
      setSelectedQuestionId(id);
      // Reset the animation timeline when opening a new question
      reset();
    }
  };
  ...
  return (
    ...
    >
      ...
      {isDesktop && (
        <div
          ref={rightPanelRef}
          className={`order-2 flex-shrink-0 md:p-0 ${isResizing ? '' : 'transition-[width,transform] duration-500 ease-out'}`}
          style={{
            width: `${100 - leftWidth}%`,
            minWidth: '600px',
